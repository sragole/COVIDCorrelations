...
begin
	...
	county_case_dates = Date.(county_case_data[!, "date"][1:end-3])
	county_cases = county_case_data[!, "cases"][1:end-3]						
	
	county_deaths = county_case_data[!, "deaths"][1:end-3]
end;
...
@bind lag Slider(5:30, default=15, show_value=true)
...
@bind hosp_lag Slider(1:20, default=10, show_value=true)

# ╔═╡ 30323758-44c0-11eb-0df3-1ddd46872b74
md"""
If we assume the average ICU patient spends 7 days there, we predict that ~$(round(icu_rate*100/7)) % of identified COVID cases need ICU beds.
...
begin
	plot(county_hosp_dates[hosp_start_ind:end], 
		county_non_icu[hosp_start_ind:end], label= "State Data")
